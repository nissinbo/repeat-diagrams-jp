...
ui <- fluidPage(
  ...
  tags$div(a("Diagrams for longitudinal study designs", href = "https://presc.sdu.dk/repeat-diagrams/"), 
           "(", 
           a("ソースコード", href = "https://source.coderefinery.org/lcl/repeat-diagrams"), 
           ")", 
           " を改変し、日本語入力と pptx 出力に対応した。"
           ), 
        ...
        tabsetPanel(
          ...
          tabPanel("Output",
            ...
            fluidRow(
              ...
              column(2, br(), downloadButton("build_pptx", "Download pptx"))
            ),
            htmlOutput("pdf")
          )
...
# Lines -------------------------------------------------
  ...
  output$build_pptx <- downloadHandler(
    filename = function(){"diagram.pptx"},
    content = function(file) {
      ...
      plt_out <- gen_long_diag(ggdf(), 
                               ...
                               txt_size = input$textsize,
                               addpad = input$xpad)
      graph2ppt(plt_out, file = file, aspectr = asp)
    }
  )
